...
local CAT_ORDER = { "mex", "fusion", "solar", "geo", "windtidal", "radar", "sonar", "jammer", "storage", "converter", "nano", "factory" }
local CAT_LABELS = {
    mex = "Metal Extractors", fusion = "Fusion Reactors", solar = "Solar Collectors",
    geo = "Geothermal", windtidal = "Wind / Tidal",
    radar = "Radar", sonar = "Sonar", jammer = "Jammer",
    storage = "Storage", converter = "Converter", nano = "Nano Turret", factory = "Factories",
}
local CAT_STAT = {
    mex = "Extract", fusion = "E/tick", solar = "E/tick", geo = "E/tick",
    windtidal = "Wind/Tidal",
    radar = "Radar", sonar = "Sonar", jammer = "Jam Range",
    storage = "Capacity", converter = "Conv Cap", nano = "Build Spd",
}

local BUILDING_ARCHS = { Efficient = true, Fortified = true, Overclocked = true }

--------------------------------------------------------------------------------
-- View modes (filter buttons)
--------------------------------------------------------------------------------

local VIEW_MODES  = { "eco", "bots", "vehicles", "naval", "hover", "air", "defence" }
local VIEW_LABELS = {
    eco = "Economy", bots = "Bots", vehicles = "Vehicles",
    naval = "Naval", hover = "Hovercraft", air = "Aircraft",
    defence = "Static Defence",
}
local UNIT_VIEWS  = { bots = true, vehicles = true, naval = true, hover = true, air = true }
local MOBILE_TABS = { "bots", "vehicles", "naval", "hover", "air" }

local function classifyFactoryName(name)
    if not name then return nil end
    local rest = name
    for _, prefix in ipairs({ "arm", "cor", "leg" }) do
        if rest:sub(1, #prefix) == prefix then
            rest = rest:sub(#prefix + 1); break
        end
    end
    if rest:find("gant") then return "gantry" end
    if rest == "alab" or rest == "lab" then return "bots" end
    if rest == "avp"  or rest == "vp"  then return "vehicles" end
    if rest == "asy"  or rest == "sy"  then return "naval" end
    if rest == "aap"  or rest == "ap"  then return "air" end
    if rest == "ahp" or rest == "hp" then return "hover" end
    return nil
end

local function getTier(def)
    if def.customParams and def.customParams.techlevel then
        local t = tonumber(def.customParams.techlevel)
        if t then
            if t < 1 then return 1 end
            if t > 3 then return 3 end
            return t
        end
    end
    return 1
...
local function factionTotal(d)
    if not d then return 0 end
    local n = #d.eco
    for _, mob in ipairs(MOBILE_TABS) do
        for t = 1, 3 do n = n + #d.units[mob][t] end
    end
    for t = 1, 3 do n = n + #d.defences[t] end
    return n
end

local function getMobilityFallback(def)
    if not def then return nil end
    local tc = type(def.tedClass) == "string" and def.tedClass:upper() or ""
    if tc == "KBOT" then return "bots" end
    if tc == "TANK" then return "vehicles" end
    if tc == "SHIP" or tc == "BOAT" then return "naval" end
...
local function getCategory(def)
    ...
    if def.weapons and #def.weapons > 0 then return nil end
    if (def.extractsMetal or 0) > 0 then return "mex" end
    local cp = def.customParams
    if cp and cp.energyconv_capacity then return "converter" end
    if (def.windGenerator or 0) > 0 or (def.tidalGenerator or 0) > 0 then return "windtidal" end
    ...
    if def.buildOptions and #def.buildOptions > 0 then return "factory" end
    if def.builder or def.isBuilder then return "nano" end
    local ms = def.metalStorage or 0
    local es = def.energyStorage or 0
...
local function getCategoryStat(def, cat)
    ...
    if cat == "windtidal" then
        ...
        if w > 0 and t > 0 then return fmtNum(w) .. "/" .. fmtNum(t) end
        if w > 0 then return fmtNum(w) end
        return fmtNum(t)
    end
    if cat == "radar" then return fmtNum(def.radarDistance or 0) end
    if cat == "sonar" then return fmtNum(def.sonarDistance or 0) end
    if cat == "jammer" then return fmtNum(def.radarDistanceJam or 0) end
    if cat == "storage" then
        local ms = def.metalStorage or 0
        local es = def.energyStorage or 0
        return fmtNum(ms) .. "M/" .. fmtNum(es) .. "E"
    end
    if cat == "converter" then
        local cp = def.customParams
        return fmtNum(tonumber(cp and cp.energyconv_capacity or 0))
    end
    if cat == "nano" then return fmtNum(def.workerTime or def.buildSpeed or 0) end
    return ""
end

local function parsePrefix(prefixStr)
    -- Parse "[Mythical Deep Bore Efficient]" into rarity, trait, archetype
    if not prefixStr then return nil, nil, nil end
    local inner = prefixStr:match("^(.+)$") or prefixStr
    -- Extract base rarity name
    local rarityName, rIdx = extractBaseRarity(inner)
    if not rarityName then return inner, nil, nil end
    local rest = inner:sub(#rarityName + 1)
    rest = rest:match("^%s*(.*)$") or ""
    if rest == "" then return rarityName, nil, nil end
    -- Check if last word(s) are a known archetype
    local archetype, trait = nil, nil
    for arch in pairs(BUILDING_ARCHS) do
        if rest:sub(-#arch) == arch then
            archetype = arch
            trait = rest:sub(1, #rest - #arch):match("^(.-)%s*$")
            if trait == "" then trait = nil end
            break
        end
    end
    if not archetype then
        -- Might be a combat archetype or just trait
        trait = rest
    end
    return rarityName, trait, archetype
end
...
local function parseInfologRarities()
    ...
    -- Find all tweakdefs_rename_get_ready positions
    -- Only match actual marker lines: preceded by "] " (timestamp suffix), not source dumps
    local starts = {}
    local pos = 1
    while true do
        local s = content:find("tweakdefs_rename_get_ready", pos, true)
        if not s then break end
        -- Real markers have "] " just before; source dumps have code characters
        if s >= 3 and content:sub(s-2, s-1) == "] " then
            starts[#starts+1] = s
        end
        pos = s + 1
    end
    ...
    -- Use the first start of the last cluster (blocks from the same game load)
    local last = starts[#starts]
    local clusterStart = last
    for i = #starts, 1, -1 do
        if last - starts[i] < 500000 then
            clusterStart = starts[i]
        else
            break
        end
    end

    -- Find the last rename_end after the cluster start
    local lastEnd = clusterStart
    pos = clusterStart
    while true do
        local e = content:find("tweakdefs_rename_end", pos, true)
        if not e then break end
        lastEnd = e + #"tweakdefs_rename_end"
        pos = e + 1
    end

    local block = content:sub(clusterStart, lastEnd)
...
local function collectData()
    ...
    local function ensureFac(faction)
        if not facData[faction] then
            local d = {
                eco = {}, ecoByCategory = {}, factories = {},
                units = { bots = {}, vehicles = {}, naval = {}, hover = {}, air = {} },
                defences = {},
            }
            for _, mob in ipairs(MOBILE_TABS) do
                for t = 1, 3 do d.units[mob][t] = {} end
            end
            for t = 1, 3 do d.defences[t] = {} end
            facData[faction] = d
        end
    end
    ...
    -- Identify factories and their mobility class
    local factoryMobility = {}
    for defID, def in pairs(UnitDefs) do
        if def.buildOptions and #def.buildOptions > 0 and (def.speed or 0) == 0 then
            factoryMobility[defID] = classifyFactoryName(def.name or "") or "other"
        end
    end

    -- Build unit -> mobility maps.
    -- unitMobility: assigned by regular factory (lab/vp/sy/hp/ap)
    -- gantryUnits: built by a T3 gantry, shown in every mobile tab
    local unitMobility = {}
    local gantryUnits = {}
    for facID, mob in pairs(factoryMobility) do
        local fac = UnitDefs[facID]
        if fac and fac.buildOptions then
            if mob == "gantry" then
                for i = 1, #fac.buildOptions do
                    gantryUnits[fac.buildOptions[i]] = true
                end
            elseif mob and mob ~= "other" then
                for i = 1, #fac.buildOptions do
                    local uid = fac.buildOptions[i]
                    if not unitMobility[uid] then unitMobility[uid] = mob end
                end
            end
        end
    end
    ...
    for defID, def in pairs(UnitDefs) do
        ...
        if factoryMobility[defID] then
            -- It's a factory; skip from the new tier-based views
        elseif (def.speed or 0) > 0 then
            -- Mobile combat unit
            local dps, range = getWeaponStats(def)
            local entry = {
                rarity = rarity, rarityIdx = rIdx,
                name = cleanName, desc = desc, defID = defID,
                metalCost = round(def.metalCost or 0),
                hp = round(def.health or 0),
                speed = round(def.speed or 0),
                dps = dps, range = range, tier = tier,
            }
            if gantryUnits[defID] then
                -- T3 gantry units appear in every mobile tab
                for _, mob in ipairs(MOBILE_TABS) do
                    local bucket = facData[faction].units[mob][tier]
                    bucket[#bucket+1] = entry
                end
            else
                local mob = unitMobility[defID] or getMobilityFallback(def)
                if mob and facData[faction].units[mob] then
                    local bucket = facData[faction].units[mob][tier]
                    bucket[#bucket+1] = entry
                end
            end
        elseif def.weapons and #def.weapons > 0 then
            -- Static unit with weapons -> defence
            local dps, range = getWeaponStats(def)
            local bucket = facData[faction].defences[tier]
            bucket[#bucket+1] = {
                rarity = rarity, rarityIdx = rIdx,
                name = cleanName, desc = desc, defID = defID,
                metalCost = round(def.metalCost or 0),
                hp = round(def.health or 0),
                speed = 0, dps = dps, range = range, tier = tier,
            }
        else
            -- Static, unarmed -> eco categorization
            local cat = getCategory(def)
            if cat and cat ~= "factory" then
                local _, trait, archetype = parsePrefix(rarity)
                local catStat = getCategoryStat(def, cat)
                local entry = {
                    rarity    = rarity,
                    rarityIdx = rIdx,
                    name      = cleanName,
                    ...
                    trait     = trait,
                    archetype = archetype,
                    output    = getEcoOutput(def) or "",
                }
                facData[faction].eco[#facData[faction].eco+1] = entry
                if not facData[faction].ecoByCategory[cat] then
                    facData[faction].ecoByCategory[cat] = {}
                end
                facData[faction].ecoByCategory[cat][#facData[faction].ecoByCategory[cat]+1] = entry
            end
        end
    end

    -- Sort and store
    local rarityDesc = function(a, b) return a.rarityIdx > b.rarityIdx end
    for faction, data in pairs(facData) do
        for _, entries in pairs(data.ecoByCategory) do
            table.sort(entries, rarityDesc)
        end
        for _, mob in ipairs(MOBILE_TABS) do
            for t = 1, 3 do table.sort(data.units[mob][t], rarityDesc) end
        end
        for t = 1, 3 do table.sort(data.defences[t], rarityDesc) end
    ...
    for _, f in ipairs(factionList) do
        ...
        local nu, nd = 0, 0
        for _, mob in ipairs(MOBILE_TABS) do
            for t = 1, 3 do nu = nu + #d.units[mob][t] end
        end
        for t = 1, 3 do nd = nd + #d.defences[t] end
        spEcho("[RaritiesViewer] " .. f .. ": " .. #d.eco .. " eco, " .. nu .. " mobile, " .. nd .. " defences")
    end
end
...
buildDisplayRows = function()
    ...
    if activeFilter == "eco" then
        for _, cat in ipairs(CAT_ORDER) do
            if cat ~= "factory" then
                local entries = data.ecoByCategory and data.ecoByCategory[cat]
                if entries and #entries > 0 then
                    local label = (CAT_LABELS[cat] or cat) .. " (" .. #entries .. ")"
                    addRow({ type = "section", label = label, h = SECTION_H })
                    addRow({ type = "eco_header", h = HEADER_H, category = cat })
                    for _, e in ipairs(entries) do
                        addRow({ type = "eco", rarity = e.rarity, rarityIdx = e.rarityIdx,
                                 name = e.name, desc = e.desc, defID = e.defID,
                                 metalCost = e.metalCost, hp = e.hp,
                                 catStat = e.catStat, category = cat,
                                 trait = e.trait, archetype = e.archetype, h = ROW_H })
                    end
                end
            end
        end
    elseif UNIT_VIEWS[activeFilter] then
        local list = data.units[activeFilter]
        for tier = 1, 3 do
            local entries = list and list[tier]
            if entries and #entries > 0 then
                local label = "Tier " .. tier .. " " .. (VIEW_LABELS[activeFilter] or activeFilter)
                              .. " (" .. #entries .. ")"
                addRow({ type = "section", label = label, h = SECTION_H })
    ...
    elseif activeFilter == "defence" then
        for tier = 1, 3 do
            local entries = data.defences and data.defences[tier]
            if entries and #entries > 0 then
                local label = "Tier " .. tier .. " Static Defence (" .. #entries .. ")"
                addRow({ type = "section", label = label, h = SECTION_H })
                addRow({ type = "unit_header", h = HEADER_H })
                for _, u in ipairs(entries) do
                    addRow({ type = "unit", rarity = u.rarity, rarityIdx = u.rarityIdx,
                             name = u.name, desc = u.desc, defID = u.defID,
                             metalCost = u.metalCost, hp = u.hp,
                             speed = u.speed or 0, dps = u.dps, range = u.range, h = ROW_H })
                end
            end
        end
    end
...
local ECO_COLS = {
    { name = "Rarity",    w = 160, align = "l" },
    { name = "Name",      w = 170, align = "l" },
    { name = "Archetype", w = 100, align = "l" },
    { name = "Trait",     w = 130, align = "l" },
    { name = "Metal",     w = 70,  align = "r" },
    { name = "HP",        w = 70,  align = "r" },
    { name = "Stat",      w = 100, align = "r" },
}
...
local function drawPanel()
    ...
    local contentTop = py + ph - TITLE_H - TAB_H - FILTER_H
    local contentBot = py
    ...
    -- Filter row
    local filterY = py + ph - TITLE_H - TAB_H - FILTER_H
    glColor(0.08, 0.09, 0.15, 0.95)
    glRect(px, filterY, px + pw, filterY + FILTER_H)

    local filterX = px + 4
    for _, mode in ipairs(VIEW_MODES) do
        local label = VIEW_LABELS[mode] or mode
        local fw = mathMax(textWidth(label, 12) + 18, 90)
        if mode == activeFilter then
            glColor(0.22, 0.30, 0.45, 1)
        else
            glColor(0.12, 0.14, 0.22, 0.9)
        end
        glRect(filterX, filterY + 3, filterX + fw, filterY + FILTER_H - 3)
        filterX = filterX + fw + 3
    end
    ...
    for _, mode in ipairs(VIEW_MODES) do
        ...
        local fw = mathMax(textWidth(label, 12) + 18, 90)
        if mode == activeFilter then
            bufText("\255\255\255\255" .. label, filterX + fw * 0.5, filterY + 8, 12, "oc")
        else
            bufText("\255\150\155\175" .. label, filterX + fw * 0.5, filterY + 8, 12, "oc")
        end
        filterX = filterX + fw + 3
    end
    ...
    for _, row in ipairs(displayRows) do
        ...
        elseif row.type == "eco_header" then
            ...
            local cols = {}
            for _, c in ipairs(ECO_COLS) do cols[#cols+1] = { name = c.name, w = c.w, align = c.align } end
            -- Replace generic "Stat" with category-specific label
            if row.category and CAT_STAT[row.category] then
                cols[#cols].name = CAT_STAT[row.category]
            end
        ...
        elseif row.type == "eco" then
            local rCol = rarityColor(row.rarityIdx or 0)
            local vals = { row.rarity or "?", row.name or "?", row.archetype or "—", row.trait or "—",
                           fmtNum(row.metalCost or 0), fmtNum(row.hp or 0), row.catStat or "" }
            local colors = { rCol, "\255\220\225\240", "\255\100\200\255", "\255\255\200\100",
                             "\255\190\195\210", "\255\190\195\210", "\255\140\200\140" }
            local cx = px + ECO_INDENT + ICON_COL_W
...
function widget:MousePress(x, y, button)
    ...
    -- Filter buttons
    local filterY = py + ph - TITLE_H - TAB_H - FILTER_H
    if y >= filterY and y <= filterY + FILTER_H then
        local filterX = px + 4
        for _, mode in ipairs(VIEW_MODES) do
            local label = VIEW_LABELS[mode] or mode
            local fw = mathMax(textWidth(label, 12) + 18, 90)
            if inRect(x, y, filterX, filterY, filterX + fw, filterY + FILTER_H) then
                activeFilter = mode
                buildDisplayRows()
                return true
            end
